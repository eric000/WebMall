<template>
  ...
  <div>
      ...
      <div class="checkout-page">
        ...
        <div class="container">
            ...
            <div class="addr-list-wrap">
                <div class="addr-list">
                <ul>
                    <li v-for="(item,index) in addressListFilter" @click="checkIdex=index;selectedAddressId=item.postCode" v-bind:class="{'check':checkIdex==index}">
                    <dl>
                        <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                        <a href="javascript:;" class="addr-del-btn" @click="delAddressConfirm(item.postCode)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                        </a>
                    </div>
                    <div class="addr-opration addr-set-default" >
                        <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.postCode)"><i>Set default</i></a>
                    </div>
    ...
    <modal :mdShow="isMdshow2" @close="closeModal">
        <p slot="message">地址列表至少需要有一条数据，已无法继续删除!</p>
        <div slot="btnGroup">
...
export default {
    ...
    methods:{
        init(){
            axios.get('/users/adressList').then((result)=>{
                let res = result.data;
        ...
        expand(){
            if(this.limit == 3){
                this.limit = this.addressList.lenght;
            }else{
                this.limit = 3;
            }
        },
